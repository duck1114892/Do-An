...
import { callOrder } from '../controller/api';
...
const Expressing = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const res = await callOrder(username);
                console.log('ldasdsadsad', res)
                let value = res.array
        ...
        <div style={{ height: '600px', overflowY: 'auto' }}>
            ...
            <List
                ...
                renderItem={(item) => (
                    ...
                    >
                        <List.Item.Meta
                            title={<Popover content={content} title="Sản Phẩm"> <a>{item.id} - {item.name} - Email:{item.email} - Địa Chỉ: {item.address} - Ngày Giao:{item.deliveryDate} | Dự Kiến: {item.expectedDateOfReceipt}</a> </Popover>}
